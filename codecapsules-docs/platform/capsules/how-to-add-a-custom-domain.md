# Add and Remove Custom Domains and Subdomains
...
{% hint style="danger" %}
Ensure you enter only a **root domain**. Do not include a subdomain. A root domain contains only a domain name and top-level domain, for example, `example.com`. See the [notes on understanding URL structure](how-to-add-a-custom-domain.md#understanding-url-structure) below.
{% endhint %}
...
Retrieve the static IP address you [previously copied and saved](how-to-add-a-custom-domain.md#steps-to-add-a-custom-domain) and add an A record pointing to it. Your A record entry may look something like this:
...
* **Wildcard subdomains are not supported.** You need to [create a separate CNAME record](how-to-add-a-custom-domain.md#create-a-cname-record) for each subdomain you add.
{% endhint %}
